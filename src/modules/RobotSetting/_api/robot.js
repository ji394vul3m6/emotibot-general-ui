...
export default {
  getFunctionsStatus() {
    return axios.get(GET_FUNCTIONS_INFO_PATH);
  },
  setFunctionStatus(functionKey, status) {
    const param = {
      status,
    };

    return axios.post(`${SET_FUNCTION_INFO_PATH}/${functionKey}`, param);
  },
  setFunctionInfos(infos) {
    const param = {};
    infos.forEach((info) => {
    ...
    return axios.post(SET_ALL_FUNCTIONS_INFO_PATH, param);
  },

  getRobotQAList(page, perPage) {
    const params = {};
    ...
    // return axios.get(GET_QA_LIST_PATH, { params });
  },

  rebuildRobotQAModel() {
    return axios.post(REBUILD_QA_PATH);
  },

  updateRobotQA(info) {
    return axios.post(`${QA_OPERATION_PATH}/${info.id}`, info);
  },

  getRobotChatList() {
    return axios.get(CHATS_INFO_PATH);
  },

  updateRobotChat(param) {
    return axios.post(CHATS_INFO_PATH, param);
  },
};
