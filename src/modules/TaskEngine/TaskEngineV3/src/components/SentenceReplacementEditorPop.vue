...
<div id="sentence-replacement-editor-pop" class="sentence-replacement-editor-pop">
  <h1 class="header-title">{{$t("task_engine_v3.sentence-replacement-editor-pop.title")}}</h1>
  <div class="pattern-text-container">
    <div class="pattern_column">{{$t("task_engine_v3.sentence-replacement-editor-pop.pattern")}}</div>
    <div class="text_column">{{$t("task_engine_v3.sentence-replacement-editor-pop.text")}}</div>
  </div>
  <div v-for="(re_parser, index) in re_parsers" :key="index">
    <div class="pattern-text-container">
      <div class="pattern_column">
        <input type="text" class="no_border full_width"
          :placeholder="$t('task_engine_v3.sentence-replacement-editor-pop.pattern_placeholder')"
          v-model="re_parser.pattern"
        >
      </div>
      <div class="text_column">
        <input type="text" class="no_border full_width"
          :placeholder="$t('task_engine_v3.sentence-replacement-editor-pop.text_placeholder')"
  ...
  <button class="btn-basic"
    @click="addREParser"
  >{{$t("task_engine_v3.sentence-replacement-editor-pop.add_replacement_rule")}}</button>
</div>
</template>
